...
def getting_available_rooms(
    ...
    available_rooms: Select = select(Rooms.id).select_from(
        Rooms
    )  # получение свободных  комнат
    if hotel_id:
        available_rooms = available_rooms.filter_by(hotel_id=hotel_id)
    rooms_ids_for_hotel = select(Rooms.id).subquery(name="available_rooms")
    
    return free_rooms, rooms_ids_for_hotel


def get_info_available_rooms(
    date_from: date, date_to: date, hotel_id: int | None = None, only_ids: bool = True
):
